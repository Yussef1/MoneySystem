package de.yussef.money.asyncmysql;
...
public class AsyncMySQL {
    ...
    public AsyncMySQL(Plugin owner, String host, int port, String username, String password, String database) {
        try {
            ...
            plugin = owner;
            Bukkit.getConsoleSender().sendMessage("§eEs wurde eine Datenbank gefunden. MySQL ist nun Verbunden.");
        } catch (Exception exception) {
            Bukkit.getConsoleSender().sendMessage("§4Die MySQL Daten stimmen nicht, trage in die config.yml die Daten ein.");
        }
    }
